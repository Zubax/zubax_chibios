...
def _configure_stderr_logging(verbosity):
    ...
    formatter = logging.Formatter('{}%(asctime)s %(levelname)-.1s %(name)s:{} %(message)s'
                                  .format(colorama.Style.BRIGHT + colorama.Fore.BLUE, colorama.Style.RESET_ALL),
                                  datefmt='%H:%M:%S')
...
def main() -> int:
    ...
    try:
        t = tester.Tester(iface, uid, os.getcwd(), args.valid_fw_dir, args.invalid_fw_dir, args.nid)
        t.run()
    except Exception as ex:
